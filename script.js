...
function displayIssues(issues) {
    ...
    issues.forEach(issue => {
        ...
        const labelChips = (issue.labels || []).map(label => {
            ...
            if (normalized === "bug") labelColor = "bg-yellow-100 text-yellow-700";
            else if (normalized === "help wanted") labelColor = "bg-red-100 text-red-600";
            else if (normalized === "enhancment" || normalized === "enhancement") labelColor = "bg-green-100 text-green-600";
            else if (normalized === "documentation") labelColor = "bg-blue-50 text-blue-700";
            else if (normalized === "good first issue") labelColor = "bg-purple-50 text-purple-700";
            return `<span class="text-[11px] font-semibold ${labelColor} rounded-full px-3 py-1 inline-flex items-center">${labelText}</span>`;
        }).join(" ");
        ...
        card.innerHTML = `
            <div class="px-4 py-4">
                ...
                <!-- Description -->
                <p class="text-[#64748B] text-[13px] line-clamp-2 mb-4">${issue.description || "No description provided"}</p>
            ...
            <div class="border-t border-gray-200 px-4 py-3 bg-gray-50">
                ...
                <p class="text-[#64748B] text-[12px]">
                    <strong>Date:</strong> ${new Date(issue.createdAt || issue.date).toLocaleDateString()}
                </p>
            </div>
        `;
